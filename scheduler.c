...
typedef struct {
    ...
    int total_load; // used for min-heap comparison
} Server;

Job jobs[MAX_JOBS];
Server servers[MAX_SERVERS];
int job_assignment[MAX_JOBS]; // job_assignment[i] = server_id or -1
...
int compare_job_demand(const void *a, const void *b) {
    ...
    return demandB - demandA; // descending order
}

int compare_server_load(const void *a, const void *b) {
    Server *sA = (Server *)a;
    Server *sB = (Server *)b;
    return sA->total_load - sB->total_load; // ascending order
}
